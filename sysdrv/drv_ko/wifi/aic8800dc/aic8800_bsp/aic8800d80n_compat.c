                               ...
                               char *filename, char *version_str, int version_size)
{
    ...
    if (version_str) {
        ...
        int char_idx = 0;
        int hdr_size = (int)(dst[3] & 0xFFF);
        if (hdr_size) {
            bin_str = (char *)&dst[0] + hdr_size;
        } else {
            int max_cnt = 512;
            char *tmp = bin_str;
            while (max_cnt--) {
...
int aicwf_plat_cinit_exec_8800d80n(struct aic_sdio_dev *rwnx_hw)
{
    ...
    if (chip_sub_id == 1) {
        ...
        ret = rwnx_plat_bin_fw_upload_android(rwnx_hw, ROM_FMAC_CINIT_ADDR_8800D80N_U02, RWNX_MAC_FW_CINIT_NAME_8800D80N_U02C);
    }
    if (ret) {
...
int aicwf_plat_calib_exec_8800d80n(struct aic_sdio_dev *rwnx_hw)
{
    ...
    if (chip_sub_id == 1) {
        ...
        ret = rwnx_plat_bin_fw_upload_android(rwnx_hw, ROM_FMAC_CALIB_ADDR_8800D80N_U02, RWNX_MAC_FW_CALIB_NAME_8800D80N_U02C);
    }
